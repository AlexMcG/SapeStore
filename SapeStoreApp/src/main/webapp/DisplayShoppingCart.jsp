			...
			<table class="table table-condensed">
				...
				<tbody>
					<c:if test="${cartSize != 0}">
						<c:forEach items="${ShoppingCart.cartItems}" var="current">
							<tr id="row-${current.isbn}-${current.type}">
								...
								<td class="cart_description">
									...
									<c:if test="${current.type == 'PURCHASED'}">
										<td id="bookprice-${current.isbn}-${current.type}"
											class="cart_price">$${current.bookPrice}</td>
									</c:if>
									<c:if test="${current.type == 'RENTED'}">
										<td id="bookprice-${current.isbn}-${current.type}"
											class="cart_price">$${current.rentPrice}</td>
									</c:if>
						...
						<tr>
							<td><a class="btn btn-default check_out" id="checkout"
								href="/SapeStore/checkout">Check Out</a></td>
						</tr>
